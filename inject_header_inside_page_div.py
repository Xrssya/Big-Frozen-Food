...
def run():
    ...
    with registry.cursor() as cr:
        ...
        print("=== INJECTING TOP COMPANY HEADER INSIDE PAGE DIV ===")
        ...
        target_marker = '<div class="page mb-4">'
        if target_marker in arch:
            new_arch = arch.replace(target_marker, target_marker + '\n' + header_box_html)
            inv_view.with_context(lang='id_ID').write({'arch': new_arch})
            print(" Injected top_company_header_box right inside <div class='page mb-4'>!")
        else:
            # Fallback before <h2>
            if '<h2>' in arch:
                new_arch = arch.replace('<h2>', header_box_html + '\n<h2>')
                inv_view.with_context(lang='id_ID').write({'arch': new_arch})
                print(" Fallback: Injected header box before <h2>!")
        ...
        pdf_path = '/home/adi-purwanto/Downloads/FINAL_TOP_HEADER_INV_00014.pdf'
        with open(pdf_path, 'wb') as f:
            f.write(pdf_content)
        print(f" Generated test PDF at {pdf_path}")

        cr.commit()
        print("=== SUCCESS: TOP COMPANY HEADER INJECTED AT VERY TOP OF PAGE DIV ===")
