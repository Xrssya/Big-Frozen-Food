...
class PurchaseOrderLine(models.Model):
    ...
    def _onchange_product_packaging_sync_qty(self):
        """When user selects dynamic packaging or changes packaging qty, calculate total product_qty."""
        for line in self:
            if line.product_packaging_id and line.product_packaging_qty:
                ratio = line.product_packaging_id.qty or 1.0
                line.product_qty = line.product_packaging_qty * ratio
        ...
        for line in self:
            if line.product_packaging_id:
                ...
                if ratio > 0 and line.product_qty:
                    line.product_packaging_qty = round(line.product_qty / ratio, 2)
                    line.qty_box = line.product_packaging_qty
            else:
                ratio = line.pcs_per_box or (line.product_id.pcs_per_box if line.product_id else 12) or 1
                if ratio > 0 and line.product_qty:
    ...
    def _onchange_product_id_sync_box(self):
        ...
        for line in self:
            if line.product_id:
                packagings = line.product_id.packaging_ids.filtered(lambda p: p.purchase) or line.product_id.packaging_ids
                if packagings:
                    line.product_packaging_id = packagings[0]
                    line.product_packaging_qty = 1.0
                    line.product_qty = packagings[0].qty or 1.0
                    line.qty_box = 1.0
                else:
