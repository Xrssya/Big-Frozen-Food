...
def run():
    ...
    with registry.cursor() as cr:
        ...
        print("=== FIXING ARCH_DB VIA JSON IN ODOO 18 ===")
        ...
        arch_json = json.dumps({"en_US": clean_arch, "id_ID": clean_arch})
        cr.execute("UPDATE ir_ui_view SET arch_db = %s WHERE id = %s", (arch_json, inv_view.id))
        print(" Updated arch_db JSON in SQL directly.")
        ...
        pdf_path = '/home/adi-purwanto/Downloads/PERFECT_SINGLE_HEADER_00018.pdf'
        with open(pdf_path, 'wb') as f:
            f.write(pdf_content)
        print(f" Generated test PDF at {pdf_path}")

        cr.commit()
        print("=== SUCCESS: SINGLE GLOBAL HEADER AUDITED & VERIFIED ===")
