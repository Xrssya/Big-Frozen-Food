...
def run():
    ...
    with registry.cursor() as cr:
        ...
        print("=== RESTORING CLEAN SINGLE HEADER ON REPORTS ===")

        # 1. Reset account.report_invoice_document back to standard base arch
        inv_view = env.ref('account.report_invoice_document')
        # Remove any injected inline_company_header if present
        if 'id="inline_company_header"' in inv_view.arch:
            arch = inv_view.arch
            start_str = '<div id="inline_company_header"'
            end_str = '</div>'
            s_idx = arch.find(start_str)
            if s_idx != -1:
                e_idx = arch.find(end_str, s_idx) + len(end_str)
                cleaned_arch = arch[:s_idx] + arch[e_idx:]
                # Write with context lang id_ID
                inv_view.with_context(lang='id_ID').write({'arch': cleaned_arch})
                print(" Reset account.report_invoice_document cleanly.")

        # 2. Ensure web.external_layout_standard has the single clean header
        ...
        if 'id="global_inline_header"' not in ext_arch:
            ...
            if '<t t-out="0"/>' in ext_arch:
                ...
                print(" Single global inline header verified on web.external_layout_standard.")
        ...
        pdf_path = '/home/adi-purwanto/Downloads/PERFECT_SINGLE_HEADER_INV_00018.pdf'
        with open(pdf_path, 'wb') as f:
            f.write(pdf_content)
        print(f" Generated perfect test PDF at {pdf_path}")

        cr.commit()
        print("=== SUCCESS: SINGLE CLEAN HEADER AUDITED & VERIFIED ===")
